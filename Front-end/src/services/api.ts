...
export async function fetchApi<T>(
  ...
  try {
    ...
    console.log('fetchApi sending headers:', headers);
...
export const apiService = {
  ...
  users: {
    ...
    deleteMyAccount: async (): Promise<ApiResponse<{ message: string }>> => {
      ...
      console.log('Delete Account: token from localStorage:', token);
      if (!token) {
        return { data: null, error: 'No token found', loading: false };
      }
      // Double-check header format
      const headers = {
        Authorization: `Bearer ${token}`
      };
      console.log('Delete Account: sending headers:', headers);
      return fetchApi('/me', {
        method: 'DELETE',
        headers
      });
